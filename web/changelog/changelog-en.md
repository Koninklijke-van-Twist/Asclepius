...
When ICT marks a ticket **resolved**, Save first asks for the technical solution. That note is optional and is stored as an internal (ghost) message for colleagues. You can close the prompt without saving; the ticket then stays unchanged. Under **Preferences** you can turn that prompt off; you then get a reminder to document the technical solution in the ticket yourself.
...
A URL to another ticket (such as `index.php?open=943`) becomes a clickable **Ticket #943** link in the message text.
...
On the **ICT overview** you can enable ghost mode when replying (ghost button next to the keyboard). Those messages are only visible to ICT on that overview, with purple styling and a wavy border. Status changes and other system notes remain normal messages.
