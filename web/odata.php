 ...
 * Mímir beheert environments — Asclepius heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
 ...
 *
 * max_age-beleid:
 * - nightly.php / hourly.php in Asclepius doen géén BC-OData (ticket snapshots / theevraagje).
 * - ASCLEPIUS_NIGHTLY_MAX_AGE (14400) en ASCLEPIUS_HOURLY_MAX_AGE (1800) zijn gereserveerd
 *   voor eventuele toekomstige OData-paden via die endpoints.
 * - UI / on-demand houdt de bestaande odata_get_all-TTL (default 300 s).
 */

/** Mímir max_age for nightly-style OData fetches (4h). Asclepius nightly.php does not fetch OData today. */
const ASCLEPIUS_NIGHTLY_MAX_AGE = 14400;

/** Mímir max_age for hourly-style OData fetches (≤30 min). Asclepius hourly.php does not fetch OData today. */
const ASCLEPIUS_HOURLY_MAX_AGE = 1800;
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'Asclepius-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_api_key() !== '') {
        // Mímir beheert de BC-cache (max_age); Asclepius-filecache / live-paginering worden overgeslagen.
